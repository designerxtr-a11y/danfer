...
export async function uploadPassportPhoto(token: string, formData: FormData, travelerIndex?: number) {
  ...
  if (file.type && !ALLOWED_TYPES.includes(file.type)) {
    return { ok: false as const, error: "Please upload a photo or PDF of your passport or ID" };
  }

  const ext = (file.name.split(".").pop() ?? "jpg").toLowerCase();
  const travelerPrefix = travelerIndex ? `traveler-${travelerIndex}-` : "";
  const path = `${token}/${travelerPrefix}${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
