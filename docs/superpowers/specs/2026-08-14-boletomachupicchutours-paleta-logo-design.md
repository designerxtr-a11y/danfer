...
Estas no se repintan con el cambio de tokens CSS porque no usan clases Tailwind — hay que editarlas a mano con los valores nuevos:
...
- `src/app/opengraph-image.tsx`, `src/app/[locale]/(public)/blog/[slug]/opengraph-image.tsx`, `src/app/[locale]/(public)/tours/[slug]/opengraph-image.tsx`: fondo/gradiente `#0B1929`/`#060F1C` → `#01173E`/`#000C24`. Todo el texto que hoy está en `#E8B043`/`#C8901E` (el span `"BOLETO MP"` del wordmark, el precio grande, el rating con estrella) pasa a `#FD4707` — en los tres archivos ese texto siempre se renderiza sobre el fondo navy de la propia imagen OG, nunca sobre blanco, así que cumple la regla 3 (5.10:1, pasa AA) sin necesitar la excepción de logotipo.
...
- Grep de los hex viejos (`#0B1929`, `#060F1C`, `#0077B6`, `#023E8A`, `#C8901E`, `#E8B043`) en `src/` debe dar 0 resultados.
